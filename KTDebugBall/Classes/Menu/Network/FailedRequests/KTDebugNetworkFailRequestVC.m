...
@interface KTDebugNetworkFailRequestVC () <UITableViewDelegate, UITableViewDataSource>
...
@implementation KTDebugNetworkFailRequestVC

- (void)viewDidLoad
{
	[super viewDidLoad];
	self.title = @"Failed Request";
...
- (void)reloadData
{
	...
	for (KTHttpLogModel *model in array) {
		if (model.statusCode.integerValue == 200 && !model.business_error) {
			continue;
		}
		
		KTHttpLogModel *failed = [model yy_modelCopy];
		failed.spread = NO;
		[failedRequests addObject:failed];
	}
	
	self.datas = failedRequests;
